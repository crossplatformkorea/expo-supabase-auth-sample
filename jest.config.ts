...
export default async (): Promise<Config.InitialOptions> => {
  return {
    ...
    transformIgnorePatterns: [
      ...
      'node_modules/(?!((jest-)?react-native|@react-native(-community)?)|expo(nent)?|@expo(nent)?/.*|expo-router|@expo-router/.*|@unimodules/.*|unimodules|sentry-expo|native-base|react-native-svg|dooboo-ui|@dooboo-ui)',
    ],
    modulePaths: ['<rootDir>'],
